...
/* =========================================================================
   AssignCheck — homepage interactions
   ========================================================================= */
...
['home', 'features', 'how', 'teachers', 'students', 'about'].forEach(id => {
...
new IntersectionObserver((entries, obs) => {
  entries.forEach(en => {
    if (en.isIntersecting) { dash.classList.add('in'); obs.disconnect(); }
  });
...
function firePulse(i) {
  ...
  dot.animate(
    [
      ...
      { transform: `translate(${to.x}px,${to.y}px) scale(.5)`,     opacity: .9 }
    ],
...
function resetStage() {
  stage.classList.remove('p-upload', 'p-compare', 'p-match', 'p-result', 'p-reset');
...
async function runCycle() {
  ...
  const pulses = setInterval(() => { firePulse(pulseIndex % 4); pulseIndex++; }, 260);
  await wait(2000);
  ...
  matches.forEach((l, i) => setTimeout(() => l.classList.add('found'), 100 + i * 140));
  await wait(matches.length * 140 + 600);
  stage.classList.add('p-result');
  setStatus('Analysis complete', 'done');
  barEl.style.width = '100%';
  $$('.rrow', engine).forEach((row, i) => setTimeout(() => {
...
(async function loop() {
  for (;;) {
    if (!stageVisible) { await wait(500); continue; }
    await runCycle();
  }
})();
...
// Update connection lines from each t-doc to teacher-ref-card
function updateTeacherLinks() {
  ...
  const refCenter = {
    x: ((refRect.left + refRect.width / 2) - stageRect.left) / stageRect.width * 100,
    y: ((refRect.top + refRect.height / 2) - stageRect.top) / stageRect.height * 100
  };
  ...
  tDocs.forEach((doc, i) => {
    ...
    const cx = ((r.left + r.width / 2) - stageRect.left) / stageRect.width * 100;
    const cy = ((r.top + r.height) - stageRect.top) / stageRect.height * 100;
...
window.addEventListener('resize', () => setTimeout(updateTeacherLinks, 150));
window.addEventListener('load', updateTeacherLinks);
setTimeout(updateTeacherLinks, 200);

new IntersectionObserver(entries => entries.forEach(en => {
  teacherVisible = en.isIntersecting;
  if (en.isIntersecting && !teacherRunning) { teacherRunning = true; runTeacherCycle(); }
}), { threshold: 0.2 }).observe(teacherWrap);

async function runTeacherCycle() {
  while (true) {
    if (!teacherVisible) { await wait(500); continue; }
    ...
    // Results — reveal matches one by one
    ...
    tDocs.forEach((d, i) => {
      setTimeout(() => {
        ...
        if (refCountEl) refCountEl.textContent = matchedCount + '/' + tDocs.length;
      }, i * 180);
    });
...
new IntersectionObserver(entries => entries.forEach(en => {
  ...
  if (en.isIntersecting && !studentRunning) { studentRunning = true; runStudentCycle(); }
}), { threshold: 0.2 }).observe(studentWrap);
...
async function runStudentCycle() {
  ...
  while (true) {
    if (!studentVisible) { await wait(500); continue; }
    ...
    // Comparing phase — documents move toward engine
    studentStage.classList.add('p-comparing');
    await wait(2400);
    
    // Highlight matching lines (indices 1, 3, 4 — same in both documents)
